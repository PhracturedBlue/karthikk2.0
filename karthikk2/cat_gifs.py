...
CAT_DIR = os.path.join(TOP_DIR, "images", "cat_gifs")
...
class CatGifs:
    ...
    def do_cats(self, *args):
        """Hypno-cat wrapper"""
        self.cbhandler.visual_exec(self.hypno_cat)

    def hypno_cat(self, scr):
        """Hypno-cat"""
        width, height = scr.get_size()
        scr.fill(0)
        pygame.display.flip()
        images = glob(os.path.join(CAT_DIR, "*.gif"))
        cat = GIFImage(random.choice(images))
        count = int(6 * self.fps)
        clock = pygame.time.Clock()
        w = cat.get_width()
        h = cat.get_height()
        while count: 
            cat.render(scr, [(width - w) / 2, (height - h) / 2])
            pygame.display.flip()
            clock.tick(self.fps)
            count -= 1
