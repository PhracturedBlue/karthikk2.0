...
class HotwordDetector(object):
    ...
    def saveMessage(self):
        ...
        """
        tempfile = 'temp' + str(int(time.time())) + '.wav'
        filename = TOP_DIR + '/temp/output' + str(int(time.time())) + '.wav'
        data = b''.join(self.recorded_data)

        #use wave to save data
        wf = wave.open(tempfile, 'wb')
        wf.setnchannels(1)
        ...
        wf.writeframes(data)
        wf.close()
        os.system("sox --norm=-3 {} {}".format(tempfile, filename))
